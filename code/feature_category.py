...
print('*'*20,'合并category_cnt计算','*'*20)
res=[]
for part in tqdm(range(5)):
    tmp_df_00=pd.read_feather(input_path+str(part)+'_itemcategory_cnt.feather')
    tmp_df_00=pl.DataFrame(tmp_df_00)
    res.append(tmp_df_00)
tmp_df_00=pl.concat(res)
tmp_df_00=tmp_df_00.groupby(['Item_Category','日期','Behavior_Type']).sum()
tmp_df_00=tmp_df_00.rename({'count':'cnt'})
df_cnt=tmp_df_00.select(['Item_Category','日期','Behavior_Type','cnt'])
# tmp_df_00=tmp_df_00.to_pandas(use_pyarrow_extension_array=True)
# tmp_df_00.to_feather(input_path+'itemcategory_cnt.feather')
print('*'*20,'合并category_cnt完成','*'*20)
del res
...
def create_feat_func(df_cnt,out_file_name,pk_list,col_name):
    ...
    train_df_01_01=df_cnt.pivot(values="cnt", index=pk_list, columns="Behavior_Type", aggregate_function="sum")
...
# # 品类特征
create_feat_func(df_cnt,train_or_test+'_df_04_01.feather',['Item_Category'],'品类')
create_last_n_days_feat1(df_cnt,train_or_test+'_df_04_06.feather',['Item_Category'],'品类',10)

print('feature_category 全部完成')