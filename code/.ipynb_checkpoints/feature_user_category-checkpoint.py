...
def create_last_n_days_feat(in_file_name,out_file_name,pk_list,col_name,start_day_num=1,n_days=3):
    ...
    print(out_file_name,'已完成')
# 衰减求和后N天的计数
def create_last_n_days_feat1(in_file_name,out_file_name,pk_list,col_name,n_days=3,xishu=1):
    train_df_01_01_res=[]
    for n_day in tqdm(range(1,n_days+1)):
        start_day=arrow.get(pred_date).shift(days=-1*n_day).format('YYYY-MM-DD')
        l1=os.listdir(output_path)
        l1=[i for i in l1 if i[:len(in_file_name)]==in_file_name and i[-18:-8]<pred_date and i[-18:-8]>=start_day]
        l1=sorted(l1)
        train_df_01_01=pd.read_feather(output_path+l1[0])
        train_df_01_01=train_df_01_01[pk_list].copy()
        for idx,file in enumerate(l1):
            tmp_df=pd.read_feather(output_path+file)
            cols=[i for i in list(tmp_df) if i not in pk_list]
            tmp_df[cols]=tmp_df[cols]*(1/(1+xishu*(n_day-idx)))
            train_df_01_01=train_df_01_01.merge(tmp_df,'outer',pk_list)
        train_df_01_01=train_df_01_01.fillna(0)
        for col in [col_name+'浏览次数',col_name+'收藏次数',col_name+'加购次数',col_name+'购买次数']:
            cols=[i[-18:-8]+'_'+col for i in l1]
            train_df_01_01[col]=train_df_01_01[cols].sum(axis=1)
            for i in l1:
                del train_df_01_01[i[-18:-8]+'_'+col]
        for i in l1:
            del train_df_01_01[i[-18:-8]+'_'+col_name+'总交互次数']
        l2=[col_name+'浏览次数',col_name+'收藏次数',col_name+'加购次数',col_name+'购买次数']
        combs= combinations(range(4),2)
        for tmp_comb in combs:
            col1=l2[tmp_comb[0]]
        ...
        train_df_01_01.columns=new_cols
        train_df_01_01_res.append(train_df_01_01)
    train_df_01_01=train_df_01_01_res[0].copy()
    for tmp_df in tqdm(train_df_01_01_res[1:]):
        train_df_01_01=train_df_01_01.merge(tmp_df,'outer',pk_list)
    train_df_01_01.fillna(0,inplace=True)
    train_df_01_01.to_feather(output_path+out_file_name)
    print(out_file_name,'已完成')
# 求和后N天的计数
...
def create_last_n_days_feat2(in_file_name,out_file_name,pk_list,col_name,n_days=3,xishu=1):
    ...
    train_df_01_01=pd.read_feather(output_path+l1[0])
    train_df_01_01=train_df_01_01[pk_list].copy()
    for idx,file in tqdm(enumerate(l1)):
        tmp_df=pd.read_feather(output_path+file)
        del tmp_df[file[-18:-8]+'_'+col_name+'总交互次数']
        cols=[i for i in list(tmp_df) if i not in pk_list]
        tmp_df[cols]=tmp_df[cols]*(1/(1+xishu*(n_days-idx)))
        train_df_01_01=train_df_01_01.merge(tmp_df,'outer',pk_list)
    train_df_01_01.fillna(0,inplace=True)
    for col in [col_name+'浏览次数',col_name+'收藏次数',col_name+'加购次数',col_name+'购买次数']:
        cols=[i[-18:-8]+'_'+col for i in l1]
...
# # 用户品类特征
create_feat_func('user_category_cnt.feather',train_or_test+'_df_05_01.feather',['User_ID','Item_Category'],'用户品类')# 这里有点慢
...
df_03_09('user_category_last_time.feather',train_or_test+'_df_05_09.feather',['User_ID','Item_Category'],'品类')
create_last_n_days_feat('user_category_cnt.feather',train_or_test+'_df_05_10.feather',['User_ID','Item_Category'],'用户品类',1,5)
create_last_n_days_feat3('user_category_cnt.feather',train_or_test+'_df_05_11.feather',['User_ID','Item_Category'],'用户品类',10)

print('feature_part1 全部完成')
